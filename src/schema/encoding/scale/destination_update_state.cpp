#include <charter/schema/encoding/scale/chain.hpp>
#include <charter/schema/encoding/scale/destination_update_state.hpp>

using namespace charter::schema;

namespace charter::schema::encoding::scale {

void encode(destination_update_state<1>&& o, ::scale::Encoder& encoder) {
  encode(o.version, encoder);
  encode(o.workspace_id, encoder);
  encode(o.destination_id, encoder);
  encode(o.update_id, encoder);
  encode(o.type, encoder);
  encode(o.chain_type, encoder);
  encode(o.address_or_contract, encoder);
  encode(o.enabled, encoder);
  encode(o.label, encoder);
  encode(o.created_by, encoder);
  encode(o.created_at, encoder);
  encode(o.not_before, encoder);
  encode(o.required_approvals, encoder);
  encode(o.approvals_count, encoder);
  encode(o.status, encoder);
}

void decode(destination_update_state<1>&& o, ::scale::Decoder& decoder) {
  decode(o.version, decoder);
  decode(o.workspace_id, decoder);
  decode(o.destination_id, decoder);
  decode(o.update_id, decoder);
  decode(o.type, decoder);
  decode(o.chain_type, decoder);
  decode(o.address_or_contract, decoder);
  decode(o.enabled, decoder);
  decode(o.label, decoder);
  decode(o.created_by, decoder);
  decode(o.created_at, decoder);
  decode(o.not_before, decoder);
  decode(o.required_approvals, decoder);
  decode(o.approvals_count, decoder);
  decode(o.status, decoder);
}

}  // namespace charter::schema::encoding::scale
