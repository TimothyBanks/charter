...
template <uint16_t Version> struct create_policy_set;

template <> struct create_policy_set<1> final {
  uint16_t version{1};
  ...
  std::vector<std::pair<role_id_t, std::vector<public_key_t>>> roles;
